...
int fork_pid(pid_t pid)
{
	...
	if (flock(fd, LOCK_EX)) {
		log_stderr("Can't lock pid file");
		close(fd);
		return -1;
	}
	char buf[32];
	...
	if (new_pid < 0) {
		...
		if (ret != -1 && new_pid != pid) {
			log_error("Forked [ %d ] instead of [ %d ]", new_pid, pid);
			ret = -1;
		}
		if (write(pipefd[1], &ret, sizeof(ret)) != sizeof(ret)) {
			log_stderr("Can't write to pipe");
			ret = -1;
		}
		close(pipefd[1]);
		return ret;
		...
		if (read(pipefd[0], &ret, sizeof(ret)) != sizeof(ret))
			log_stderr("Can't read from pipe");
		if (ret < 0) {
			log_info("Exiting from child due to parent err");
			exit(1);
		}
		close(pipefd[0]);
		return 0;
